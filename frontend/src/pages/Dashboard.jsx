...
function Dashboard() {
    ...
    const handleLoadScannedItem = async (overrideId = null) => {
        // Use overrideId if provided, otherwise fallback to state
        // Note: overrideId might be an event object if called from onClick without args, so check type
        const idToLoad = (typeof overrideId === 'string' ? overrideId : scannedItemId);

        if (!idToLoad) return;

        // Check if the item is already loaded
        if (scannedItem && scannedItem._id === idToLoad) {
        ...
        setError('');
        try {
            const item = await apiRequest(`/items/${idToLoad}`);
            setScannedItem(item);
            setQuantityInput(item.quantity.toString()); // Initialize quantity input
            // If we used an override ID (from scanner), also update the input field state to match
            if (typeof overrideId === 'string') {
                setScannedItemId(overrideId);
            }
        } catch (err) {
            setScannedItem(null);
            setQuantityInput('');
        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-50  via-indigo-50/40 to-purple-50/30">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <main className="space-y-6">
                    {/* Premium Add Item Form - Only for logged in users */}
                    <section className="flex justify-center">
                        {token ? (
                            ...
                            <div className="w-full max-w-2xl bg-gradient-to-br from-blue-600 via-indigo-600 to-purple-600 rounded-3xl p-8 shadow-2xl text-center text-white">
                                <span className="text-5xl mb-4 block">👋</span>
                                <h2 className="text-3xl font-extrabold mb-4">Welcome to Scanventory</h2>
                                <p className="text-lg opacity-90 mb-8">
                                    You are viewing the public inventory. To add your own items or manage stock levels, please create an account or log in.
                                </p>
                                <div className="flex flex-col sm:flex-row gap-4 justify-center">
                                    <a
                                        href="/login"
                                        className="rounded-xl bg-white px-8 py-3 text-sm font-bold text-indigo-600 shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105"
                                    >
                                        🔓 Log In
                                    </a>
                                    <a
                                        href="/signup"
                                        className="rounded-xl bg-indigo-500 border-2 border-white/50 px-8 py-3 text-sm font-bold text-white shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-105"
                                    >
                                        ✨ Create Profile
                                    </a>
                                </div>
                    ...
                    <section className="bg-gradient-to-br from-emerald-50 via-teal-50 to-cyan-50 rounded-3xl border-2 border-emerald-200/50 p-8 shadow-2xl backdrop-blur-sm">
                        ...
                        <div className="flex flex-col sm:flex-row gap-3">
                            ...
                            <button
                                ...
                                className="rounded-xl bg-white border-2 border-emerald-600 px-6 py-3 text-sm font-bold text-emerald-700 shadow-lg hover:shadow-xl hover:bg-emerald-50 focus:outline-none focus:ring-4 focus:ring-emerald-300/50 transition-all duration-300 transform hover:scale-105 active:scale-95"
                            >
                                📷 Camera
                            </button>
                        </div>
                            ...
                            <QRScanner
                                onScan={(data) => {
                                    if (data) {
                                        ...
                                        // Directly load the scanned item
                                        handleLoadScannedItem(data);
                                    }
                                }}
                                ...
                                <div className="flex flex-col lg:flex-row items-start justify-between gap-6">
                                    <div className="flex-1">
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            {/* Quantity Input Field - Only for logged in users */}
                                            {token && (
                                                <div className="flex items-center gap-3">
